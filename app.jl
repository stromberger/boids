...
n = 100*500 # has to be divisible by threads
vmax = 0.03
d0 = 0.1
dc = 0.02
l0 = 0.31
...
function update_boid(b, blist, nearby)
    ...
    @inbounds for i in 1:length(nearby)
        ...
        if d < d0
            ...
            # we know that the list is sorted so, when 
            # the first element is not in sight the others are even further away
            break
        end
    end
...
function start_simulation()
    ...
    if (as_arrows) 
        ...
        arrows!(ax3d, points, velocities, linewidth = 0.005, 
            arrowsize = Vec3f(0.2, 0.2, 0.3)*0.05, linecolor=strength, arrowcolor=strength)
    else
    ...
    # pre alloc buffer
    buffer = [Vector{Boid}(undef,div(n, threads)) for t in 1:threads]
    for t in 1:threads
    ...
    function update_sim() 
        ...
        nearby = ThreadsX.map((i) -> 
            hcat(
                [x[1] for x in shape[i,:]], 
                first(knn(kdtree, [x[2] for x in shape[i,:]], max_nearby_boids, true))
            ),
            1:threads)
        end
